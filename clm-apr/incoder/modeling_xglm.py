...
class XGLMModel(XGLMPreTrainedModel):
    ...
    def parallelize(self, device_map=None):
        ...
        # print(next(self.embed_tokens.parameters()).device)
    ...
    def deparallelize(self):
        ...
        self.layer_norm = self.layer_norm.to("cpu")
...
class XGLMForCausalLM(XGLMPreTrainedModel):
    ...
    def forward(
        ...
        if hasattr(self.model, "first_device"):
            logits = self.lm_head(outputs[0].to(self.model.first_device))
        else:
    ...
    def _reorder_cache(past, beam_idx):
        reordered_past = ()
        cur_device = beam_idx.device
        beam_idx_ = beam_idx
        for i, layer_past in enumerate(past):
            if layer_past[0].device != cur_device:
                cur_device = layer_past[0].device
                beam_idx_ = beam_idx_.to(cur_device)
            reordered_past += (tuple(past_state.index_select(0, beam_idx_) for past_state in layer_past),)
